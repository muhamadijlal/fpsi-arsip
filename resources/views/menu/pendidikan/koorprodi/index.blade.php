...
@endpush
@section('title',Breadcrumbs::render('koorprodi'))
...
<div class="col-lg-12 mt-3">
  <div class="card mb-4">
    <div class="card-body">
      <h5 class="card-title">Tabel data SK KOORDINATOR DAN PROGRAM STUDI</h5>
      {{-- <div class="card-subtitle text-muted mb-3">Card subtitle</div> --}}
      <a href="{{ Auth::user()->role == 'dosen' ? url('/dosen/pendidikan/koorprodi/add') : url('/root/pendidikan/koorprodi/add') }}" class="btn btn-md btn-outline-primary mb-4">
        <span class="tf-icons bx bx-plus-medical"></span>&nbsp; SK KOORDINATOR DAN PROGRAM STUDI
      </a>
...
<script type="text/javascript">
  $(function () {
    let table = $('#example').DataTable({
      ...
      ajax: "{{ route('dosen.dataTable.koorprodi') }}",
      columns: [
        {data: '', name: ''},
        {data: 'aksi', name: 'aksi'},
...
function confirmDelete(data_id) {
    ...
    .then((willDelete) => {
      if (willDelete) {
      window.location.href = ("/dosen/pendidikan/koorprodi/delete/"+data_id);
      } else {
