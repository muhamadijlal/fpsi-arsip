...
@section('title',Auth::user()->role != 'tu' ? Breadcrumbs::render('KP.Mahasiswa.dinas.create') : Breadcrumbs::render('KP.Tu.dinas.create'))
...
<div class="card">
  <div class="card-body">
    <div class="row">
      <div class="col">
        <form action="{{ Auth::user()->role != 'mahasiswa' ? route('tu.dinas_kerja_praktik_store') : route('mahasiswa.dinas_kerja_praktik_store') }}" method="post">
          @csrf
          ...
          <div class="mb-3">
              ...
              <input type="text" class="form-control @error('instansi') is-invalid @enderror" name="instansi" id="instansi" value="{{old('instansi')}}" autocomplete="off">
              @error('instansi')
              <div class="text-danger">
                  {{ $message }}
              </div>
